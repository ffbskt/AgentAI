...
class AlibiTransformerLayer(nn.Module):
    ...
    def ensure_mask_like(self, x):
        ...
        alibi_method = os.environ.get("ALIBI_METHOD")
        if alibi_method == "exp":
        ...
        elif alibi_method == "softmax":
            mask = mask[None] * torch.softmax(self.ms, 0)[:, None, None]
            if random.random() < 1e-3:
                print(
                    self.level,
                    torch.softmax(self.ms, 0).sort().values.round(decimals=3).detach(),
                )
        elif alibi_method == "single":
            mask = (
                mask[None]
                * (
                    2
            ...
            mask = mask[None] * (2 ** -torch.arange(self.num_heads))[:, None, None].to(
                x.device
            )
